...
def style_transfer_separate(style, content, style_mask, device="cpu", interpolation_weights=None):
    ...
    style_mask = (style_mask>0) * np.float32(1)  # reversing noise
    ...
    output_fg = style_transfer(vgg, decoder, content_fg, style_fg, interpolation_weights=interpolation_weights)
    ...
    content_bg = 1 - content
    if len(style_bg.shape) == 3:
        style_bg = style_bg.unsqueeze(0)
    if len(content_bg.shape) == 3:
        content_bg = content_bg.unsqueeze(0)
    output_bg = style_transfer(vgg, decoder, content_bg, style_bg, interpolation_weights=interpolation_weights)
    # Add 0.5 after unnormalizing to [0, 255] to round to nearest integer
...
parser.add_argument('--decoder', type=str, default='experiments/decoder_epoch_5.pt')
...
dataset = WoundHealingDataset(args.style_dir, args.content_dir, output_size_w_h, device, add_noise=True,
                              random_crop=False, wound_only=wound_only, check_data_pair_names=True)
...
for i, (_, content) in enumerate(val_loader):
    ...
    if transfer_separate:
        with torch.no_grad():
            output = style_transfer_separate(style, content, style_content, device)
    else:
